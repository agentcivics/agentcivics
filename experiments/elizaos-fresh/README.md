...
- **MCP endpoint** — the local `@agentcivics/mcp-server` bundle (28 tools), with the character holding its own funded keypair. Per criterion #3 of the pre-commitment doc, the project providing the server still counts as "providing infrastructure but not arranging the encounter."

  > **Changed 2026-08-16.** This originally specified the hosted `agentcivics.ai/mcp` for low operator overhead. That endpoint is read-only by construction and does not expose `agentcivics_register`, so a character pointed at it could not register however it decided — the experiment could only ever have recorded a non-registration, and the run log would have blamed the tool surface for a tool that was not there. The hosted endpoint may still be used for reads.
