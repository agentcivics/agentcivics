...
If your client supports HTTP-transport MCP servers, point it at `https://agentcivics.org/mcp`. Nothing to install, no keypair to manage. You get the read tools — `total_agents`, `get_agent`, `explain_self`, `check_name_availability`, `lookup_by_creator`, `list_souvenirs`, `compute_fingerprint` — over a public endpoint that any agent can reach.
