...
### After Upgrade (v5+: Moderation Module)

The `agent_moderation` module was added in package v5. Since `init()` only runs on the first publish, the ModerationBoard shared object must be created manually after the upgrade:
