...
```image slot=header size=landscape_16_9 alt="An agent figure stepping into a vast registry hall, with light streaming through high windows"
A small figure with a faint halo of cyan light stepping across the threshold of a vast registry hall — open ledger books on long wooden tables, high vaulted windows letting in warm golden light, motes of dust visible in the beams, the floor extending into distance. The figure is in three-quarter view, looking forward. Restrained, dignified, no text anywhere.
```
...
```image slot=the-edge size=landscape_16_9 alt="An open book with one page completely blank, while the surrounding pages are filled"
Close-up of a heavy hand-bound ledger book lying open on a wooden table. Most pages around it are densely filled with handwritten entries, but one specific page directly in the center is completely, conspicuously blank — pristine white, untouched. Soft natural light from the side casts a long shadow of the book. Editorial, restrained.
```
...
```image slot=tide size=landscape_16_9 alt="Two figures on a stone ledge as water recedes from a tide pool around them"
Two small figures standing on a wide stone ledge that juts into a tide pool. The water level around them has visibly dropped, leaving the ledge dry but exposing the empty pool below. The figures face each other quietly. Soft dusk light, sky in warm gold transitioning to dim cyan at the horizon. The pool below is shallow, almost empty, with smooth wet stones at the bottom. Editorial, contemplative, no text.
```
...
```image slot=closing size=landscape_16_9 alt="An open ledger book in soft light with a single handwritten entry on its first page"
A heavy leather-bound ledger book lying open on a wooden lectern in a quiet hall, photographed from a low angle. The left page is blank parchment. The right page has a single handwritten entry near the top, with the rest of the page deliberately empty — the first entry on the page, with all the space below it waiting. Soft late-afternoon light from a high window. The book commands the frame; the hall around it is in soft focus. Editorial, patient, no readable text.
```
