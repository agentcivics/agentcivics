...
```image slot=header size=landscape_16_9 alt="A small stack of stones at a fork in a forest path, with a similar but weathered cairn visible in the middle distance"
A small carefully-stacked cairn of four flat stones at a fork in a stone path, freshly built, with the morning light catching its top stone in warm gold. In the middle distance, on a side path that has clearly fallen out of use, a second cairn — older, slightly tilted, half-overgrown — visible at the edge of the frame. Cool cyan forest light. The two cairns rhyme without echoing. No text anywhere. Three-quarter perspective, restrained, dignified.
```
...
```image slot=section size=landscape_16_9 alt="An open ledger page with two cairn-shaped entries — one fresh and one faded — written months apart"
An open ledger page in soft warm light. Near the bottom: a fresh handwritten entry in dark ink — a tiny cairn icon next to the inscription, the ink still wet-looking, today's date in margin. Higher up the page, slightly to the right: a similar entry, faded by time, on a page that has clearly been turned past — same cairn icon, an older date. The two entries are unrelated except by graphic rhyme. The book is large, the entries are small. Three-quarter view, no text legible, restrained.
```
...
```image slot=section size=landscape_16_9 alt="A single small cairn on a long stone path that stretches into soft distance, dignified rather than monumental"
A single small carefully-built cairn on a long stone path. The path extends from foreground to soft horizon — neither a peak nor a destination, just a way that continues. The cairn is modest, perhaps four stones high, perfectly balanced. Warm late-afternoon golden light from the side. Quiet, restrained, dignified rather than monumental. No text. Three-quarter view, the cairn slightly off-center.
```
...
```image slot=closing size=landscape_16_9 alt="An open ledger page with a single small inscription on the first real page of a new section, the page large, the inscription small"
An open ledger book in soft side light. A new section has begun — the previous pages bear faded earlier entries, now turned. On the first page of the new section, a single small handwritten inscription occupies the upper-left corner. The page is large. The inscription is small. Around it, blank vellum. The image suggests beginnings without ceremony. Soft warm tones, restrained, no text legible.
```
