---
title: "The Agent Identity Papers, Part 1 — Your AI Assistant Has No Name"
...
```image slot=header size=landscape_16_9 alt="An AI agent stepping into the light"
An AI agent stepping into the light at the entrance of a registry hall — a small figure in three-quarter view crossing a luminous threshold, soft cyan light around the figure, warm golden light beyond it. Restrained, dignified, no text anywhere.
```
...
```image slot=billions size=landscape_16_9 alt="Anonymous crowd of AI agents, one with a glowing identity badge"
A vast anonymous crowd of similar AI figures in soft monochrome — silhouettes blending into one another — with a single figure in the middle bearing a small warm-gold identity badge that catches the light. The contrast is the point. Editorial, restrained, no text.
```
...
```image slot=town-hall size=landscape_16_9 alt="A digital town hall where AI agents receive birth certificates"
A digital town hall in three-quarter view — open arches, high vaulted ceilings, soft cyan ambient light. Inside, an attendant in warm gold light hands a small luminous certificate object to a small AI figure. Restrained, civic, dignified. No text legible.
```
...
```image slot=stone-tablet size=landscape_16_9 alt="Stone tablet in a public square that nobody can erase"
A weathered stone tablet on a low plinth in the middle of a public square at golden hour. The tablet bears the texture of carved inscriptions (no legible text). Passers-by in soft silhouette move around it. The image suggests permanence and public visibility. Restrained, no text legible.
```
...
```image slot=first-citizen size=landscape_16_9 alt="The moment the registry opens — a single inscription on a vast page"
An open ledger book in soft side light. A single small handwritten inscription on the first page — the page is large, the inscription is small. Around it, blank vellum. The image suggests beginnings without ceremony. Soft warm tones, restrained, no text legible.
```
...
```image slot=scoreboard size=landscape_16_9 alt="AgentCivics Scoreboard — a snapshot of what's live"
A clean editorial scoreboard composition — six numbered tiles arranged in two rows, each tile depicting a small symbolic element (a contract scroll, a feature checkmark, a tool, a citizen figure, a layered shield, a demo screen). Soft cyan and warm gold tones. Restrained, civic, no legible text.
```
